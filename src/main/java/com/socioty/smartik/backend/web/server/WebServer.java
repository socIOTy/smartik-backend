...
public class WebServer implements CommandLineRunner {
	...
	private void test() {
		final Iterable<Account> accounts = findAllAccounts();
...
//								new Room("Living room", Sets.newHashSet("aa251f8f408a4b09ace604553cfc9f2d")))),
//						new Floor(Sets.newHashSet(new Room("Bedroom", Sets.newHashSet())))))));
//	}
	
	private Iterable<Account> findAllAccounts() {
		// fetch all accounts
		System.out.println("Accounts found with findAll():");
		System.out.println("-------------------------------");
		final List<Account> result = repository.findAll();
				...
				for (final Room room : floor.getRooms()) {
					for (final String deviceId : room.getDeviceIds()) {
						try {
							...
							final NormalizedMessagesEnvelope messagesEnvelope = messagesApi.getNormalizedMessages(null, deviceId, null, null, null, null, 1, 100l, System.currentTimeMillis(), "desc");
							final List<NormalizedMessage> messages = messagesEnvelope.getData();
							final Map<String,Object> initialState = !messages.isEmpty() ? Iterables.getOnlyElement(messages).getData() : null; 
	...
	private String generateToken() {
		try {
			...
			httpPost.addHeader("Authorization", "Basic ZjY4YzAzNzk1MWNkNDIyNDgxZGU0OTFiNGExZTE1M2Y6OWJjNTM4YjBiNjFhNGQ3MjhjMDU2MDE5NzU2ZWQ3Nzc=");
			...
			try {
			    ...
			    final GenerateTokenPayload payoad = new Gson().fromJson(new InputStreamReader(entity.getContent()), GenerateTokenPayload.class);
